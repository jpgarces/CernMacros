...
void fstate_ppions(){
  ...
  TFile *outf = new TFile("ppions_PT_sm.root","RECREATE");

  TH1F *hist_pip_PT = new TH1F("hist_pip_PT", "hist_pip_PT", 100, 0.0, 100.0);

  /*TH1F *hist_pip_tau_PT = new TH1F("hist_pip_tau_PT", "hist_pip_tau_PT", 100, 0.0, 100.0);

  TH1F *hist_pip_else_PT = new TH1F("hist_pip_else_PT", "hist_pip_else_PT", 100, 0.0, 100.0);
  ...
  for(int entry = 0; entry < allEntries; ++entry){
	...
	// hist_pip_tau_PT->Fill(particle->PT);
	hist_pip_PT->Fill(particle->PT);
          //cout << "Tau->electron pT: " << particle->PT << endl;
        //}else{
	//hist_pip_else_PT->Fill(particle->PT);
	//}
      }
  ...
  cout << "** Exiting..." << endl;
  /*hist_pip_PT->Scale(1/(hist_pip_PT->GetEntries()));
  hist_pip_tau_PT->Scale(1/(hist_pip_tau_PT->GetEntries()));
