...
void tau_PT_Rapidity_lep(){
  ...
  TFile *outf = new TFile("e_PT_Rapidity_bkg.root","RECREATE");

  TH2F *hist_e_PT_rapidity = new TH2F("hist_e_PT_rapidity","Electron P_{T} vs. Rapidity",150,0.0,15.0,150,-6.0,6.0);

  Double_t e_PT, e_rapidity;
  ...
  //for(int entry = 0; entry < 5000; ++entry){
   ...
   for(int j = 0; j < branchParticle->GetEntriesFast(); ++j){
	...
	tau_n=(GenParticle*) branchParticle->At(j);
	  ...
	  hist_e_PT_rapidity->Fill(particle->PT,particle->Rapidity);
	  //}
      }
   }
 }
