...
void tau_PT_Rapidity(){
  ...
  TFile *outf = new TFile("tau_PT_Rapidity_bsm.root","RECREATE");

  TH2F *hist_tau_PT_rapidity = new TH2F("hist_tau_PT_rapidity","Tau P_{T} vs. Rapidity",150,0.0,15.0,150,-6.0,6.0);
...
for(int entry = 0; entry < allEntries; ++entry){
  //for(int entry = 0; entry < 5000; ++entry){
   ...
   tau_PT=tau_n->PT;
   tau_rapidity=tau_n->Rapidity;
   hist_tau_PT_rapidity->Fill(tau_PT,tau_rapidity);
 }
