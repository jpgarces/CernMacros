...
void diff_RecGen(Int_t pid)
{
  ...
  Int_t rec_count_mum = 0, rec_count_mup = 0, rec_count_track=0;

  Double_t pt_cut=10.0;
  Double_t eta_cut=2.5;
  Int_t gen_count_mum=0, gen_count_mup=0;
  Int_t i, j, k, diff_mum=0, diff_mup=0, diff_track=0;
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry){
   //if (entry>20000) break;
    ...
    k=0;
    i=0;
    while(i < branchParticle->GetEntriesFast()){
 ...
 cout<< "Number of generated (negative) muons: " << gen_count_mum << endl;
 cout << "Number of reconstructed (negative) muons (Muon branch): " << rec_count_mum << endl;
 //cout << "Number of reconstructed (negative) muons (Track branch): " << rec_count_track << endl;
 cout << "Gen - Rec (Muon branch): " << diff_mum << endl;
