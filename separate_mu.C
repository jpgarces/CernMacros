...
void separate_mu(){
  ...
  TFile *outf = new TFile("muons_PT_sm_sep.root","RECREATE");

  TH1F *hist_tau_mu_PT = new TH1F("hist_tau_mu_PT", "hist_tau_mu_PT", 100, 0.0, 100.0);
  TH1F *hist_else_mu_PT = new TH1F("hist_else_mu_PT", "hist_else_mu_PT", 100, 0.0, 100.0);
  ...
  for(int entry = 0; entry < allEntries; ++entry){
	...
	hist_tau_mu_PT->Fill(particle->PT);
          //cout << "Tau->electron pT: " << particle->PT << endl;
        }else{
	  hist_else_mu_PT->Fill(particle->PT);
	}
       }
     }
  }
  cout << "** Exiting..." << endl;
  //hist_tau_mu_PT->Scale(1/(hist_tau_mu_PT->GetEntries()));
  //hist_else_mu_PT->Scale(1/(hist_else_mu_PT->GetEntries()));
