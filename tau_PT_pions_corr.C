...
void tau_PT_pions_corr(){
  ...
  TFile *outf = new TFile("tau_PT_pions_corr.root","RECREATE");

  TH2F *hist_pippim_PT = new TH2F("hist_pippim_PT","Charged pions",150,0.0,8.0,150,0.0,8.0);
  TH2F *hist_phph_PT = new TH2F("hist_phph_PT","Charged pions",150,0.0,8.0,150,0.0,8.0);
  ...
  //for(int entry = 0; entry < 5000; ++entry){
   ...
   for(int t=0;t<2;++t){
    ...
    if (pim_count==2&&pip_count==1&&pi0_count==0&&ph_count==0&&nut_count==1){
      hist_pippim_PT->Fill((pim_PT_arr[0]+pim_PT_arr[1])/2.0,pip_PT);
    }

    if (pim_count==2&&pip_count==1&&pi0_count==1&&ph_count==2&&nut_count==1){
      hist_phph_PT->Fill(ph_PT_arr[0],ph_PT_arr[1]);
    }
	...
	hist_ph_PT_3ch20->Fill(ph_P_arr[3]);
	}*/
  }
