...
void ditau_PT_lep_nonu(){
  ...
  TFile *outf = new TFile("ditau_PT_lep_nonu.root","RECREATE");

  TH1F *hist_ditau_eep_PT_nonu = new TH1F("hist_ditau_eep_PT_nonu","di-Tau PT from electron/positron decays",100,0.0,60.0);
  TH1F *hist_ditau_emup_PT_nonu = new TH1F("hist_ditau_emup_PT_nonu","di-Tau PT from electron/positive-muon decays",100,0.0,60.0);
  TH1F *hist_ditau_muep_PT_nonu = new TH1F("hist_ditau_muep_PT_nonu","di-Tau PT from muon/positron decays",100,0.0,60.0);
  TH1F *hist_ditau_mumup_PT_nonu = new TH1F("hist_ditau_mumup_PT_nonu","di-Tau PT from muon/positive-muon decays",100,0.0,60.0);

  int e_count, anue_count, nut_count, n, nue_count, ep_count,anut_count, mu_count, anumu_count, numu_count, mup_count;

  Double_t pt;
    
  Double_t daughter_PT_nonu[10];
   ...
   for (int t=0;t<10;++t){
    ...
    for (int i=tau_n->D1; i<= tau_n->D2; ++i){
	 ...
	 if ((daughter->PID==11 || daughter->PID==13)&&daughter->Status==1){
	    daughter_PT_nonu[n]=daughter->PT;
            n+=1;
         }
       }
     }
